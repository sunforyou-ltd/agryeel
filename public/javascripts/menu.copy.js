    ...
    var inputJson = StringToJson('{"accountId":"' + accountInfo.accountId + '", "farmId":"' + accountInfo.farmId + '"}');
      ...
      $.ajax({
        ...
        complete:function(data, status, jqXHR){						//処理成功時
            ...
            var houseList 	= jsonResult.targetCompartmentStatus;		//ハウス状況対象リスト
            ...
            //----- ここからハウス状況照会の編集 ----
            for ( var houseKey in houseList ) {                      		//対象圃場情報分処理を行う

                var houseStatus	= houseList[houseKey];	                	//ハウス状況情報の取得
                ...
                iCount++;													//件数カウントアップ
                htmlString += '<canvas id="G0002HouseStatusG' + houseKey + '" class="house-working-statusgraph"></canvas>';
                ...
                $("#G0002HouseList").append(htmlString);									//可変HTML部分に反映する
...
//                    }
//
//                    var myChart  = new Chart(document.getElementById("G0002HouseStatusG" + houseKey).getContext("2d")).Doughnut(grafhDataList);
//
//                }
...
//
//                   var myChart  = new Chart(document.getElementById("G0002HouseStatusG" + houseKey).getContext("2d")).Bar(linegrafhData);
//
//                }
